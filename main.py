...
Path(settings.FFMPEG_TEMP_DIR).mkdir(parents=True, exist_ok=True)
...
class StartWorkflowRequest(BaseModel):
    ...
    def validate_remote_inputs(self) -> "StartWorkflowRequest":
        try:
            ...
            if self.product_listing_url:
                self.product_listing_url = validate_public_http_url(
...
async def get_job_status(job_id: str) -> JobStatusResponse:
    ...
    if not message and job.get("error"):
        message = job["error"]
...
async def get_generation_status(job_id: str) -> JobStatusResponse:
    ...
    if job_id not in _generation_jobs:
        ...
        workflow_job = job_manager.get_job(job_id)
        if workflow_job:
            result = workflow_job.get("result")
            message = result.get("message") if isinstance(result, dict) else None
            if not message and workflow_job.get("error"):
                message = workflow_job["error"]
            return JobStatusResponse(
                job_id=job_id,
                ...
                result=result,
                message=message,
                created_at=workflow_job.get("created_at"),
                updated_at=workflow_job.get("updated_at"),
            )
        raise HTTPException(status_code=404, detail=f"Job {job_id} not found")
...
async def approve_keyframes(
    ...
    try:
        ...
        # 所有前置校验通过后再更新状态，避免 409 时项目错误停留在 GENERATING。
        await airtable_service.update_project_status(
            project_id=project_id,
